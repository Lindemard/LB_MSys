...
class Pathfinder {
  ...
  private def get_actions(wumpusPercept: WumpusPercept) = {
    Console.println("Мир: Агент в  [" + agentX + "; " + agentY + "]")
    Console.print("Мир: Агент был в " + agentPosition.x + " | " + agentPosition.y + "\n")
    ...
    Console.print("Мир: В комнате: " + room_info.length)
    if (room_info.isEmpty) {
      Console.println("ничего")
    }
    else {
      Console.println()
    }
    if (actions == null) {
      if (!wumpusPercept.bump) {
        ...
        if (imaginatyWorld.worldGrid(agentPosition).exist != Pathfinder.ROOM_STATUS_TRUE) {
          ...
          Console.println("MARKED THE EXISTENCE")
        }
        moveRoom = false
      }
      else {
...
//  var gold: Int = add.Pathfinder.ROOM_STATUS_NO_STATUS
...
//      case add.Pathfinder.GOLD =>
//        this.gold = add.Pathfinder.ROOM_STATUS_TRUE
